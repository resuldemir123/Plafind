...
namespace Plafind.Services
{
    ...
    public interface ICompareService
    {
        ...
        /// </summary>
        List<int> GetCompareList(ISession session);

        /// <summary>
        /// Session'a karşılaştırma listesini kaydeder
        /// </summary>
        void SaveCompareList(ISession session, List<int> businessIds);

        /// <summary>
        /// İşletmeyi karşılaştırma listesine ekler (kategori kontrolü ile)
        /// </summary>
        (bool success, string message, int count) AddToCompareList(ISession session, int businessId, int? existingCategoryId = null);

        /// <summary>
        /// İşletmeyi karşılaştırma listesinden kaldırır
        /// </summary>
        (bool success, string message, int count) RemoveFromCompareList(ISession session, int businessId);

        /// <summary>
        /// Karşılaştırma listesini temizler
        /// </summary>
        void ClearCompareList(ISession session);

        /// <summary>
        /// Karşılaştırma listesi sayısını döner
        /// </summary>
        int GetCompareListCount(ISession session);

        /// <summary>
        /// İşletme karşılaştırma listesinde mi kontrol eder
        /// </summary>
        bool IsInCompareList(ISession session, int businessId);
